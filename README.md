...
- Docker / Docker Compose（VoiceVox 用）
...
# ウェイクワードモデルファイルのパス（デフォルト: mochitto.ppn）
WAKE_WORD_PATH=mochitto.ppn
```
...
| コンテナ | Docker / docker-compose | VoiceVox Engine 用 |
...
├── docs/                       # ドキュメント・設計書
├── assets/                     # エラー音声ファイル等
├── docker-compose.yml          # VoiceVox Engine 起動用
└── pyproject.toml              # 依存管理
```
