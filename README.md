...
サーバー（開発マシン）とクライアント（Raspberry Pi）をそれぞれセットアップします。

### 前提条件

| 対象 | 必要なもの |
|------|-----------|
| サーバー（開発マシン） | Docker / Docker Compose, [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html)（GPU 利用時） |
| クライアント（Raspberry Pi） | [uv](https://docs.astral.sh/uv/), mpv, PortAudio（`libportaudio2`） |
| 共通 | Git |

### Step 1: サーバーのセットアップ（開発マシン）
...
`.env` を編集し、サーバー側の設定を記入します（詳細は[設定](#設定)セクションを参照）。
...
4. 生成された `.ppn` ファイルをダウンロード

また、Porcupine の日本語モデルファイルをダウンロードします。

- [porcupine_params_ja.pv](https://github.com/Picovoice/porcupine/blob/master/lib/common/porcupine_params_ja.pv)（GitHub から Raw ダウンロード）

この 2 ファイルは次のステップで Raspberry Pi に配置します。

### Step 3: クライアントのセットアップ（Raspberry Pi）

```bash
# Raspberry Pi にリポジトリをクローン
git clone https://github.com/hexylab/mochitto.git
cd mochitto

# ウェイクワードファイルを配置
# Step 2 でダウンロードした .ppn と .pv をプロジェクトルートにコピー
#   mochitto/mochitto.ppn
#   mochitto/porcupine_params_ja.pv

# クライアント用の依存関係をインストール
uv sync --extra client
```

`.env` ファイルを作成し、クライアント側の設定を記入します（詳細は[設定](#設定)セクションを参照）。

```bash
cp .env.example .env
# SERVER_URL と PORCUPINE_ACCESS_KEY を設定
```
...
│   └── feedback_audio/            # フィードバック音（検出音・処理中音）
│
├── tests/                         # テスト（pytest）
├── scripts/
│   ├── install-service.sh         # systemd サービス登録スクリプト
│   └── generate_error_audio.py    # エラー音声の生成
│
