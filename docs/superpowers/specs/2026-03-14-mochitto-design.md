...
| SwitchBotClient | httpx (REST API) | SwitchBot Cloud API経由でデバイス操作。TV操作もHub Mini IR経由で統合 |
...
| switchbot | tv | `power_on`, `power_off`, `volume_up`, `volume_down`, `channel_up`, `channel_down`, `mute` | SwitchBot Hub Mini IR経由。赤外線リモコンとして登録済みのHisense 55U7Hを操作 |
