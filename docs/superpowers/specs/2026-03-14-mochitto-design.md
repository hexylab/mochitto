...
| TVClient | vidaa-control | Hisense 55U7H MQTT over TLS制御 |
...
| tv | tv | `power_on`, `power_off`, `volume`, `mute`, `input_source`, `channel`, `key` | `volume`: `{ "direction": "up"\|"down" }`, `input_source`: `{ "source": str }`, `channel`: `{ "number": int }`, `key`: `{ "key_name": str }` (home/back/menu/ok/up/down/left/right等) |
...
| TV制御 | vidaa-control | MQTT over TLS |
