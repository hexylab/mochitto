...
| F5 | SwitchBot操作 | 照明（オン/オフ・明るさ）、エアコン（温度・モード）、カーテン（開閉） |
| F6 | TV操作 | Hisense 55U7Hのフルリモコン操作（電源、音量、入力切替、チャンネル等） |
| F7 | Web検索 | 「〜を調べて」でWeb検索し、結果をずんだもんの声で要約読み上げ |
| F8 | 音楽再生 | 「〜の音楽を流して」でYouTube Musicから楽曲検索・ストリーミング再生 |

### 非機能要件

- ハードウェア: Raspberry Pi（クライアント）+ EMEETスピーカーフォン、開発マシン（サーバー、GPU推奨）
- LAN内通信で低レイテンシを確保
...
| SwitchBotClient | httpx (REST API) | SwitchBot Cloud API経由でデバイス操作 |
| (TV操作はSwitchBotClient経由のIR制御に統合) | SwitchBot Hub Mini IR | Hisense 55U7H 赤外線リモコン制御 |

### LLM意図解釈

GPT-5.4の Structured Outputs（`response_format: { type: "json_schema" }`）を使い、以下のスキーマに従った出力を強制する。
...
|---|---|---|
| `device_control` | `intent`, `device_type`, `device_category`, `action`, `response` | `params`（デフォルト: `{}`） |
...
| device_type | device_category | 有効なaction | params |
|---|---|---|---|
| switchbot | light | `on`, `off`, `brightness` | `{ "brightness": 0-100 }` |
| switchbot | aircon | `on`, `off`, `set` | `{ "temperature": int, "mode": "cool"\|"heat"\|"auto" }` |
| switchbot | curtain | `open`, `close` | なし |
| switchbot | tv | `power_on`, `power_off`, `volume_up`, `volume_down`, `channel_up`, `channel_down`, `mute`, `input_source` | SwitchBot Hub Mini IR経由。赤外線リモコンとして登録済みのHisense 55U7Hを操作 |
...
SwitchBotデバイスは起動時にSwitchBot Cloud APIからデバイス一覧を取得し、`device_category`（light/aircon/curtain）でマッピングする。LLMにはデバイス一覧をシステムプロンプトに注入し、ユーザーの発話中の部屋名・デバイス名から適切なデバイスを選択させる。

```python
# 起動時にSwitchBot APIから取得し、システムプロンプトに注入する形式
devices = {
    "switchbot": [
        {"id": "DEVICE_ID_1", "name": "リビング照明", "category": "light"},
        {"id": "DEVICE_ID_2", "name": "寝室エアコン", "category": "aircon"},
        ...
    ]
}
```
...
EMEETスピーカーフォンのエコーキャンセリング機能がスピーカー出力とマイク入力を分離するため、音楽再生中でもWake Word検出は動作する想定。ただし、大音量時は検出精度が低下する可能性がある。
...
| SwitchBot API失敗 | エラー詳細をLLMに渡して応答テキスト生成 | 「照明の操作に失敗したのだ。接続を確認してほしいのだ」 |
| TV操作失敗 | 同上 | 「テレビの操作がうまくいかなかったのだ」 |
...
3. ローカルHTTPサーバー(localhost:PORT)でコールバック待受
...
model: "gpt-5.4"
tools: [{"type": "web_search"}]  // Web検索有効化
```
...
│   │   └── tv.py               # Hisense vidaa-control
│   └── config.py               # 設定
│
├── docker-compose.yml          # VoiceVox Engine起動用
...
| SwitchBot | httpx (REST API) | Hub Mini経由 |
| TV制御 | SwitchBot Hub Mini (IR) | SwitchBot Cloud API経由 |
| コンテナ | Docker / docker-compose | VoiceVox用 |
